...
상단의 `명함 제작` 탭에서 위키드 스타일의 꿈 명함을 만들 수 있습니다.

- 경찰관, 소방관, 축구선수, 영화감독, 선생님 5가지 예시 직업 배경을 제공합니다.
- `WEKID / DREAM CARD` 로고와 직업명 배지는 각 직업별 배경 이미지에 포함되어 있습니다.
- 웹에서는 한글 이름과 영어 이름만 배경 위에 합성되며, 이름은 입력창에서 바로 수정할 수 있습니다.
- 직업은 버튼으로 선택하거나 검색해서 고를 수 있습니다.
- A4 인쇄 시 실제 명함 크기인 `90mm x 50mm`로 10장이 반복 배치되어 잘라 쓸 수 있습니다.
- 명함 배경 이미지는 `public/business-card-backgrounds/`에서 관리합니다.
...
`npm run build`는 데이터 검증, 명함 제작 기능 검증, TypeScript 컴파일, Vite 빌드를 순서대로 실행합니다.
