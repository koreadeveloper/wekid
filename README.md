...
- 명함 제작 화면은 owner 관리자 로그인 상태에서만 열립니다.
- owner 관리자는 설문 응답을 불러오거나 이름·소속·연락처·목표를 직접 입력해 앞면/뒷면 꿈 명함을 만들 수 있습니다.
- 직업은 107개 목록에서 검색해 고르거나, 목록에 없는 직업명을 직접 입력할 수 있습니다.
- 위키드 로고, 고양시자원봉사센터 로고, 직업별 명함 배경, A4 10장 출력 레이아웃을 반영했습니다.
...
| owner 관리자 기능 | 응답별 문항 선택, 분야 점수, 추천 결과, 최종 꿈, CSV 조회, 꿈 명함 제작 |
| 꿈 명함 제작 | 응답 불러오기, 직접 정보 입력, 직업 검색/직접 입력, 앞면/뒷면 미리보기, A4 10장 출력 |
...
처음 화면에서 이름과 센터명을 입력하면 결과지와 관리자 조회에 함께 남습니다.
두 항목은 선택 입력이므로 비워도 검사를 시작할 수 있습니다.
...
### 8. owner 관리자 전용 꿈 명함 제작

owner 관리자로 로그인하면 상단에 `명함 제작` 탭이 표시됩니다.
이 화면에서는 이름으로 저장된 설문 응답을 찾아 불러오거나, 현장에서 명함에 들어갈 정보를 직접 입력할 수 있습니다.

명함 제작 기능은 다음 흐름을 지원합니다.

| 기능 | 설명 |
| --- | --- |
| 직업 검색 | 107개 직업 목록에서 원하는 직업을 빠르게 검색 |
| 직업 직접 입력 | 목록에 없는 직업명도 명함에 바로 반영 |
| 정보 입력 | 이름, 영문 이름, 소속, 연락처, 이메일, 한 줄 목표 입력 |
| 사진 첨부 | 사진이 있을 때만 뒷면 사진 영역 출력 |
| 앞면/뒷면 미리보기 | 위키드 로고와 센터 로고가 들어간 명함 디자인 확인 |
| 인쇄 | A4 한 장에 명함 10장을 배치하고 불필요한 점선 없이 출력 |
...
| owner 명함 제작 | 직업 검색·직접 입력, 앞면/뒷면, 사진 유무, A4 10장 출력 확인 |
